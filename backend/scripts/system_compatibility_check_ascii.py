#!/usr/bin/env python3
"""
System Compatibility Check (ASCII Compatible)
系统集成兼容性检查 (ASCII兼容版本)
"""
...
class SystemCompatibilityChecker:
    ...
    def _check_import_compatibility(self):
        ...
        for module_name in critical_modules:
            try:
                ...
                print(f"    [OK] Success - {len(import_results[module_name]['classes'])} classes, {len(import_results[module_name]['functions'])} functions")
    ...
    def _analyze_module_dependencies(self):
        ...
        if circular_dependencies:
            for cycle in circular_dependencies:
                print(f"    [WARNING] Cycle: {' -> '.join(cycle)}")
    ...
    def _test_workflow_connectivity(self):
        ...
        try:
            ...
            print("  Base workflow node: Available")
        except Exception as e:
            workflow_tests["base_node"] = {
                "status": "failed",
                "error": str(e)
            }
            print(f"  Base workflow node: Failed - {e}")
